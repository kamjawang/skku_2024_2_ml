...
class TabAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.cont_embeddings == 'MLP':
            # Adjust continuous embeddings to match `dim`
        ...
        hidden_dimensions = [dim * len(categories) + dim * num_continuous, *map(lambda t: dim * t, mlp_hidden_mults), dim_out]
    ...
    def forward(self, x_categ, x_cont):
        ...
        if self.cont_embeddings == 'MLP':
            ...
            x_cont_enc = x_cont_enc.unsqueeze(1).expand(-1, x_categ_enc.size(1), -1)
        else:
            x_cont_enc = None

        # Merge tensors
        if x_cont_enc is not None:
            x = x_categ_enc + x_cont_enc
        else:
            x = x_categ_enc
